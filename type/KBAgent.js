...
function KBAgent(domain, role, oppRole,gameid,country, newType) {
  this.domain = domain;
  this.socket;
...
KBAgent.prototype = {
  ...
  initializeKBAgent: function (){
    ...
    if (self.role == 'Employer'){
      self.posibleOpponent.push(new OpponentData('A', 'ShortTerm', 'Short', self.country, self.newType));
      self.posibleOpponent.push(new OpponentData('A', 'Compromise', 'Comp', self.country, self.newType));
      self.posibleOpponent.push(new OpponentData('A', 'LongTerm', 'Long', self.country, self.newType));
      self.agentStuff = new OpponentData('A', 'ShortTerm', 'Short', self.country, self.newType);
    }
    else {
      self.posibleOpponent.push(new OpponentData('B', 'ShortTerm', 'Short', self.country, self.newType));
      self.posibleOpponent.push(new OpponentData('B', 'Compromise', 'Comp', self.country, self.newType));
      self.posibleOpponent.push(new OpponentData('B', 'LongTerm', 'Long', self.country, self.newType));
      self.agentStuff = new OpponentData('B', 'ShortTerm', 'Short', self.country, self.newType);
    }
  ...
  checkOpponent: function(turn, offer){
    ...
    var updatedTypeProbability = 0;
      ...
      for (var i = 0; i < self.posibleOpponent.length; i++){ 
        ...
        sumProbabilities += prevOfferProbability * prevTypeProbability; 
      } 
      ...
      for (var i = 0; i < self.posibleOpponent.length; i++){
       ...
       self.posibleOpponent[0].lastOfferIndex = self.posibleOpponent[self.currOpponent].lastOfferIndex;
       self.currOpponent  = 0;
       for (var i = 0; i < self.posibleOpponent.length; i++){
        if (self.posibleOpponent[i].probability > self.posibleOpponent[self.currOpponent].probability){
          self.posibleOpponent[i].lastOfferIndex = self.posibleOpponent[self.currOpponent].lastOfferIndex;
          self.currOpponent = i;
          self.nik = self.posibleOpponent[i].nikName;
        }
      }

...
NewAgent.prototype = {
  ...
  //he brings the next issue to discuss on   
    ...
    if (!issueToTalk){
       var a_t = self.A[0];
       return("pick an issue to talk about");
    }
    else{
 ...
 function readAspirationFile (country){
  var some = new Object();
  var f = true;
  var j;
  var lines
  if(country == "egypt")
    lines = fs.readFileSync('./domains/aspiration/aspiration - egypt.xml', 'utf8');//, function (err,data){
  else
    lines = fs.readFileSync('./domains/aspiration/aspiration.xml', 'utf8');//, function (err,data){
